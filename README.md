...
Every genuinely-failed message is ingested from the tenant, enriched, investigated by an agent that decides for itself what evidence to pull, diagnosed with a hybrid confidence signal, routed through a **confidence-and-risk-gated autonomy policy**, and — if eligible — mitigated against the real tenant with evidence-based verification. A **circuit breaker** goes further: for contain-the-damage root causes (expired cert/credential, bad payload) that have failed repeatedly, it **autonomously stops the flow with no human in the loop** — the production "kill switch" pattern, threshold-gated so a single transient blip never trips it. A human-confirmed resolution for anything escalated is written back into the knowledge base, so the same failure class is recognized automatically next time. Scored end-to-end against a 20-case labeled evaluation set.

> **Design stance — bounded autonomy, routed by root cause.** The system can genuinely *act*, but the action is chosen to fit the failure, not applied blindly. Two operations are auto-executable: **STOP** (undeploy the flow) when leaving it running keeps causing harm — a sender pushing bad payloads that create wrong records, or an expired cert/credential that will only keep failing (redeploying those just repeats the failure); and **REDEPLOY** (restart + verify) for transient stuck runtime state. Everything higher-stakes — rotating a credential, patching a mapping, an architecture change — is always surfaced as a *recommendation* for a human, never auto-executed regardless of confidence, because it depends on systems or judgment outside this pipeline's trust boundary. Each action's execution type is declared in `data/action_map.json` and re-derived server-side at execution time, so autonomy is earned per-action-type by explicit policy, not assumed from a confidence score.
...
| **Circuit Breaker** | A pipeline node that runs autonomously: when a STOP-type root cause (cert/credential/bad-payload) has failed at least `auto_stop_failure_threshold` times in the lookback window, it **stops the flow with no human click** to contain the damage. Threshold-gated so a single blip never trips it; configurable/disable-able. | Autonomous kill-switch, threshold gating |
| **Executor** | For auto-approved, LOW-risk incidents, routes to the action that fits the root cause: **STOP** (undeploy) for damage-causing failures (bad payload, expired cert/credential) so it stops mis-processing further messages; **REDEPLOY + verify** for transient stuck state. Polls the live tenant to confirm the action landed, reports outcomes honestly, and refuses anything off the allowlist. Gated server-side, independent of client input. | Live infrastructure actuation, root-cause-based routing, evidence-based verification |
